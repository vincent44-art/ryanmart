...
export const fetchSellerFruits = async (token = null) => {
  ...
  const response = await api.get('/api/seller-fruits', config);
...
export const createSellerFruit = async (fruitData, token = null) => {
  try {
    const config = token ? { headers: { Authorization: `Bearer ${token}` } } : {};
  const response = await api.post('/api/seller-fruits', fruitData, config);
...
export const deleteSellerFruit = async (fruitId, token = null) => {
  try {
    const config = token ? { headers: { Authorization: `Bearer ${token}` } } : {};
  const response = await api.delete(`/api/seller-fruits/${fruitId}`, config);
