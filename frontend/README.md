...
This frontend reads the backend base URL from the environment variable `REACT_APP_API_BASE_URL` at build time.
...
- After changing any `.env` file, rebuild the app (`npm run build`) so the value is baked into the bundle.
